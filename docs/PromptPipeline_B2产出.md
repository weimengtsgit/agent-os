...
1. 建立 `control-plane/pal/`，落地 PAL 抽象接口（Runtime/LLM/Tool/Memory/Trace）
2. 建立 `agent-runtime-agno/adapters/`，实现 agno Runtime Adapter（只在这一层依赖 agno）
3. 规定 **业务 Agent 只能依赖 `platform-sdk`**（或 `control-plane` 的公共包），CI 加“禁止直接依赖 agno”规则
...
* [ ] `control-plane/pal` 接口齐全（Runtime/Tool/LLM/Memory/Event/Trace/Policy）
* [ ] 业务示例代码只依赖 `control-plane/sdk`，不依赖 agno
